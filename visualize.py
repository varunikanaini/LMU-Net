...
def visualize_and_save(model, loader, device, args, color_map):
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(loader, desc="Generating Visualizations")):
            ...
            if original_img_np.shape[2] == 1: # Handle grayscale images
...
if __name__ == '__main__':
    ...
    # --- ADJUSTED: Universal model loading logic ---
    print(f"Loading Light_LASA_Unet with backbone: '{args.backbone}'")
    model = Light_LASA_Unet(
        num_classes=args.num_classes,
        backbone_name=args.backbone
        # Assuming default LASA kernels are sufficient for visualization
    ).to(device)
