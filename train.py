...
# --- REPLACE your existing test(args) function with this one ---
...
def test(args):
    ...
    # --- Metrics Storage (Expanded) ---
    all_fold_metrics = {
        'oa': [], 'miou': [], 'fwiou': [], 'dice_per_image': [], 'mean_dice_global': [], 
        'mae': [], 'acd': [], 'sensitivity': [], 'specificity': [], 'precision': [], 'f1_score': []
    }
    ...
    for fold_idx in range(num_folds_to_test):
        ...
        with torch.no_grad():
            for sample in tqdm(test_loader, desc=f"Testing Fold {fold_idx}"):
                ...
                # Calculate new per-image metrics
        ...
        # Get metrics from your existing seg_utils.py
        ...
        mean_mae = np.mean(mae_scores) if mae_scores else 0.0 # Calculate mean MAE
        
        logging.info(f"Fold {fold_idx} Results -> OA: {acc_global:.4f}, mIoU: {miou:.4f}, Mean Dice (Global): {mean_dice_global:.4f}, MAE: {mean_mae:.4f}, "
    ...
    for key, values in all_fold_metrics.items():
        mean = np.mean(values)
        std = np.std(values)
        # Use a more readable key in the final printout
        key_name = key.replace('_', ' ').capitalize()
        logging.info(f"Average {key_name:<20}: {mean:.4f} ± {std:.4f}")
    logging.info("="*50)
