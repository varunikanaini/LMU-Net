# /kaggle/working/ARAA-Net/train.py (Corrected with Attention Gating)
...
# --- Helper Classes & Functions (Losses, Freezing, Masking, etc.) ---
...
def get_args():
    ...
    parser.add_argument('--backbone', type=str, default='mobilenet_v2', choices=['mobilenet_v2'])
    ...
    parser.add_argument('--boundary-loss-weight', type=float, default=1.5)
...
def evaluate_model(net, data_loader, device, focal_loss_fn, dice_loss_fn, args, mode="Validating"):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader, desc=mode, leave=False):
            ...
            with autocast(): # Use autocast for efficiency during validation
                outputs = net(inputs)
                total_loss = 0
...
# --- Training Function for a Single Fold (MODIFIED with Attention Gating) ---
def train_fold(args, fold_idx, train_imgs, val_imgs):
    device = torch.device("cuda")
    # Give the experiment a new name to distinguish it from the previous one
    exp_name = f"{args.backbone}_GatedAttention_LASA_{args.dataset_name.replace('TSRS_RSNA-', '').lower()}"
    ...
    logging.info(f"===== Starting Fold {fold_idx} with Attention Gating =====")
    ...
    best_ckpt_path, latest_ckpt_path = os.path.join(fold_exp_path, 'best_checkpoint.pth'), os.path.join(fold_exp_path, 'latest_checkpoint.pth')
    
    if args.resume and os.path.exists(latest_ckpt_path):
        try:
            ckpt = torch.load(latest_ckpt_path, map_location=device)
            net.load_state_dict(ckpt['model_state_dict']); optimizer.load_state_dict(ckpt['optimizer_state_dict']); scheduler.load_state_dict(ckpt['scheduler_state_dict'])
            start_epoch = ckpt['epoch'] + 1; best_mIoU = ckpt.get('best_mIoU', 0.0); patience_counter = ckpt.get('patience_counter', 0)
            logging.info(f"Resuming fold {fold_idx} from epoch {start_epoch}. Best mIoU: {best_mIoU:.4f}.")
        except Exception as e:
            logging.error(f"Could not resume fold {fold_idx}: {e}. Starting this fold from scratch."); start_epoch = 0
    ...
    for epoch in range(start_epoch, args.epochs):
        if epoch == args.fine_tune_epochs:
            unfreeze_backbone(net); new_lr = args.lr / 5.0
        ...
        for data in train_iterator:
            ...
            with autocast():
                # --- ATTENTION GATING LOGIC ---
                e1 = net.encoder1(inputs); e2 = net.encoder2(e1); e3 = net.encoder3(e2); e4 = net.encoder4(e3)
                e4_enhanced = net.lasa_module(e4); bottleneck = net.bottleneck_layer(e4_enhanced)
                mask_e1 = F.interpolate(labels_float_unsqueezed, size=e1.shape[2:], mode='nearest')
                mask_e2 = F.interpolate(labels_float_unsqueezed, size=e2.shape[2:], mode='nearest')
                mask_e3 = F.interpolate(labels_float_unsqueezed, size=e3.shape[2:], mode='nearest')
                mask_e4 = F.interpolate(labels_float_unsqueezed, size=e4.shape[2:], mode='nearest')
                gated_e1, gated_e2, gated_e3, gated_e4 = e1 * mask_e1, e2 * mask_e2, e3 * mask_e3, e4 * mask_e4
                aux_outputs = []
                d4_out = net.decoder4(torch.cat([F.interpolate(bottleneck, size=gated_e4.shape[2:], mode='bilinear', align_corners=True), gated_e4], dim=1))
                aux_outputs.append(F.interpolate(net.aux_conv_d4(d4_out), size=inputs.shape[2:], mode='bilinear', align_corners=True))
                d3_out = net.decoder3(torch.cat([F.interpolate(d4_out, size=gated_e3.shape[2:], mode='bilinear', align_corners=True), gated_e3], dim=1))
                aux_outputs.append(F.interpolate(net.aux_conv_d3(d3_out), size=inputs.shape[2:], mode='bilinear', align_corners=True))
                d2_out = net.decoder2(torch.cat([F.interpolate(d3_out, size=gated_e2.shape[2:], mode='bilinear', align_corners=True), gated_e2], dim=1))
                aux_outputs.append(F.interpolate(net.aux_conv_d2(d2_out), size=inputs.shape[2:], mode='bilinear', align_corners=True))
                d1_out = net.decoder1(torch.cat([F.interpolate(d2_out, size=gated_e1.shape[2:], mode='bilinear', align_corners=True), gated_e1], dim=1))
                aux_outputs.append(F.interpolate(net.aux_conv_d1(d1_out), size=inputs.shape[2:], mode='bilinear', align_corners=True))
                final_output = net.final_conv(d1_out)
                final_output_upsampled = F.interpolate(final_output, size=inputs.shape[2:], mode='bilinear', align_corners=True)
                outputs = tuple(aux_outputs + [final_output_upsampled])
                # --- END OF GATING LOGIC ---

                main_loss = 0
                for head_idx, pred_output in enumerate(outputs):
                    f_loss = focal_loss_fn(pred_output, labels.long()); d_loss = dice_loss_fn(pred_output, labels.long())
        ...
        if current_mIoU > best_mIoU:
            ...
            patience_counter += 1
            logging.info(f"⚠️ mIoU did not improve. Best: {best_mIoU:.4f}. Patience: {patience_counter}/{args.patience}")
        ...
        if patience_counter >= args.patience:
            logging.info(f"Early stopping triggered for fold {fold_idx}."); break
            
    logging.info(f"===== Finished Fold {fold_idx} ====="); return best_mIoU
...
def main():
    args = get_args()
    if args.test_only:
        # Construct the correct experiment name for testing
        args.exp_name = f"{args.backbone}_GatedAttention_LASA_{args.dataset_name.replace('TSRS_RSNA-', '').lower()}"
        test(args)
        return
    ...
    setup_logging(base_exp_path, 'main_training_log.log')
    logging.info(f"Starting Gated Attention experiment: '{exp_name}'\nArguments: {vars(args)}")

    if args.k_folds > 1:
        all_imgs = np.array(make_dataset(os.path.join(args.dataset_path, 'train'), args.dataset_name) + make_dataset(os.path.join(args.dataset_path, 'val'), args.dataset_name))
        ...
        if args.resume and os.path.exists(kfold_state_path):
            with open(kfold_state_path, 'r') as f:
                state = json.load(f)
                start_fold = state.get('next_fold_to_run', 0); all_fold_metrics = state.get('all_fold_metrics', {})
